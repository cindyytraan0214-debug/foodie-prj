...
export default function MenuPage() {
  ...
  const allCategories: { key: Category; label: string; emoji: string }[] = [
  ...
  const categories = allCategories.filter(
    (cat) => cat.key === 'all' || menuItems.some((item) => item.category === cat.key)
  );
              ...
              {filtered.map((item, i) => (
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    {lang === 'en' && (
                      <p className="text-[#7A0F16]/60 text-xs mt-0.5 mb-2">{item.subName || item.nameVi}</p>
                    )}
